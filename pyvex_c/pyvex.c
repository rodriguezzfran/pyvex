...
int vex_lift_multi(
	...
	for (unsigned int i = 0; i < max_blocks; i++) {
		...
		// set the direction and the address of the instruction of the block to lift
		vta.guest_bytes         = (UChar *)(insn_start);  // Ptr to actual bytes of start of instruction
		vta.guest_bytes_addr    = (Addr64)(insn_addr);

		lift_result_array[i].is_noop_block = False;
		lift_result_array[i].data_ref_count = 0;
		lift_result_array[i].const_val_count = 0;

		lift_result_array[i].irsb = LibVEX_Lift(&vta, &vtr, &pxControl); // calls the actual lifting function from VEX

		if (!lift_result_array[i].irsb) {
			// Lifting failed
			return -1;
		}

		remove_noops(lift_result_array[i].irsb);

		// Note -> I would like not to ask for this in every iteration
		if(guest == VexArchMIPS32) {
			mips32_post_processor_fix_unconditional_exit(lift_result_array[i].irsb);
		}

		get_exits_and_inst_addrs(lift_result_array[i].irsb, &lift_result_array[i]);
		get_default_exit_target(lift_result_array[i].irsb, &lift_result_array[i]); 

		if (guest == VexArchARM && lift_result_array[i].insts > 0) {
			arm_post_processor_determine_calls(lift_result_array[i].inst_addrs[0], lift_result_array[i].size, lift_result_array[i].insts, lift_result_array[i].irsb);
		}
		zero_division_side_exits(lift_result_array[i].irsb); // Note
		get_is_noop_block(lift_result_array[i].irsb, &lift_result_array[i]);

		if (collect_data_refs || const_prop) {
			execute_irsb(lift_result_array[i].irsb, &lift_result_array[i], guest, (Bool)load_from_ro_regions, (Bool)collect_data_refs, (Bool)const_prop);
		}

		// if not the last block, set the next instruction address
		if (i < max_blocks - 1) {
